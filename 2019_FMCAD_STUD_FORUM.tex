...
%
For example, to make the first clause in Example~\ref{eqn:ex} $(y_1 \lor x_1)$ a tautology, the minimum possibility is $y_1 \to 1$ and $y_1 \to \neg x_1$ (both substitutions makes the clause a tautology).
...
%
The idea \textbf{symmetry aware compilation} is to make the process of solving the problem with communication across the clauses (at multi-clause level rather than current state of doing it clausewise).
%
...
Fuzzing is an automated negative testing technique that treat software as a black-box and repeatedly “attack” it with random inputs in order to find critical defects.
%
...
%
The technique tries to systematically narrow down the particular input iteratively until a given time limit or no further narrowing is possible.
%
The reduced input can then be used for effective debugging.
%
...
%
A lack of certification is one of the critical bottlenecks in the wider application of DQBF.
%
